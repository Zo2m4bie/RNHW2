import { LOGIN_CHANGE_EMAIL, LOGIN_CHANGE_PASSWORD, LOGIN_RESET_DATA, 
    ...
    LOGIN_ERROR_WRONG_FIELDS, LOGIN_ERROR_EMPTY_FIELDS } from './types';
...
export default LoginReducer = (state = INITIAL_STATE, action) => {
    switch(action.type) {
        case LOGIN_CHANGE_EMAIL:
            return { ...state, login: action.payload, oneOfFieldsEmpty: false };
        case LOGIN_CHANGE_PASSWORD:
            return { ...state, password: action.payload, oneOfFieldsEmpty: false };
        ...
        case LOGIN_START_LOGIN:
            return { ...state, showProgress: true, oneOfFieldsEmpty: false }
        case LOGIN_ERROR_EMPTY_FIELDS:
            LayoutAnimation.configureNext(LayoutAnimation.Presets.spring);
            return { ...state, showProgress: false, oneOfFieldsEmpty: true }
    }
    return state;
}